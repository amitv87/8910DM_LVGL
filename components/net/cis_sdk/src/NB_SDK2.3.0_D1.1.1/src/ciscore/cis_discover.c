...
#define PRV_CONCAT_STR_EX(buf, len, index, str, str_len)\
{\
    if ( (len)-(index) < (str_len) ) \
        goto TAG_FAILED; \
    cis_memmove( (buf)+(index), (str), (str_len) ); \
                        ...
                        uint8_t ** bufferP )
{
    ...
    PRV_CONCAT_STR_EX( bufferLink, PRV_LINK_BUFFER_SIZE, head, LINK_ITEM_START, LINK_ITEM_START_SIZE );
    PRV_CONCAT_STR_EX( bufferLink, PRV_LINK_BUFFER_SIZE, head, LINK_URI_SEPARATOR, LINK_URI_SEPARATOR_SIZE );
    res = utils_intToText( uriP->objectId, bufferLink + head, PRV_LINK_BUFFER_SIZE - head );
    if ( res <= 0 )goto TAG_FAILED;
    head += res;
    PRV_CONCAT_STR_EX( bufferLink, PRV_LINK_BUFFER_SIZE, head, LINK_ITEM_END, LINK_ITEM_END_SIZE );
    ...
    if ( CIS_URI_IS_SET_INSTANCE( uriP ) )
    {
        ...
        subHead = 0;
        PRV_CONCAT_STR_EX( bufferLink + head, PRV_LINK_BUFFER_SIZE - head, subHead, LINK_ITEM_START, LINK_ITEM_START_SIZE );
        PRV_CONCAT_STR_EX( bufferLink + head, PRV_LINK_BUFFER_SIZE - head, subHead, LINK_URI_SEPARATOR, LINK_URI_SEPARATOR_SIZE );
        ...
        if ( res <= 0 ) goto TAG_FAILED;
        subHead += res;
        PRV_CONCAT_STR_EX( bufferLink + head, PRV_LINK_BUFFER_SIZE - head, subHead, LINK_ITEM_END, LINK_ITEM_END_SIZE );
