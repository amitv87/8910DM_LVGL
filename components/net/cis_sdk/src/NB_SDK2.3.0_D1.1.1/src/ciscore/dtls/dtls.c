                            ...
                            size_t                      buf_array_len )
{
    ...
    */
    res = CALL( ctx, cis_write, session, sendbuf, len );
        ...
        * record. */
        ...
        switch ( msg[0] )
        {
            ...
            * epoch, Finish has epoch + 1. */
            ...
            dtls_stop_retransmission( ctx, peer );
            CALL( ctx, cis_read, peer->session, data, data_length );
            break;
            default:
                             ...
                             netq_t         *node )
{
    ...
    if ( node->retransmit_cnt < DTLS_DEFAULT_MAX_RETRANSMIT )
    {
        ...
        (void)CALL( context, cis_write, node->peer->session, sendbuf, len );
        return;
    }
                           ...
                           const session_t *addr2 )
{
    ...
    /*return !cis_memcmp( addr1,
                        addr2,
                          sizeof(session_t) );*/
    dtls_debug( "dtls_session_equals:addr1->sin_family=%d,addr2->sin_family=%d,addr1->sin_port=%d,addr2->sin_port=%d,addr1->sin_addr.s_addr=%d,addr2->sin_addr.s_addr=%d\n",
        addr1->sin_family,addr2->sin_family,addr1->sin_port,
        addr2->sin_port,addr1->sin_addr.s_addr,addr2->sin_addr.s_addr);
    dtls_debug( "dtls_session_equals:addr1->sin_addr.s_addr=%d,addr2->sin_addr.s_addr=%d\n",
