        ...
        MQTTSN_topicid topic, unsigned char* payload, int payloadlen)
{
    ...
    writeMQTTSNChar(&ptr, MQTTSN_PUBLISH);      /* write message type */
    ...
    if (topic.type == MQTTSN_TOPIC_TYPE_NORMAL && qos == 3)
    {
        ...
        writeMQTTSNInt(&ptr, topic.data.long_.len); /* topic length */
    }
    else if (topic.type == MQTTSN_TOPIC_TYPE_NORMAL || topic.type == MQTTSN_TOPIC_TYPE_PREDEFINED)
        writeMQTTSNInt(&ptr, topic.data.id);
    else
    {
        writeMQTTSNChar(&ptr, topic.data.short_name[0]);
        writeMQTTSNChar(&ptr, topic.data.short_name[1]);
    }
    writeMQTTSNInt(&ptr, packetid);
    if (topic.type == MQTTSN_TOPIC_TYPE_NORMAL && qos == 3)
    {
        ...
        unsigned char returncode)
{
    ...
    ptr += MQTTSNPacket_encode(ptr, len); /* write length */
    writeMQTTSNChar(&ptr, MQTTSN_PUBACK);      /* write message type */

    writeMQTTSNInt(&ptr, topicid);
    writeMQTTSNInt(&ptr, packetid);
    writeMQTTSNChar(&ptr, returncode);
...
int MQTTSNSerialize_ack(unsigned char* buf, int buflen, unsigned short packet_type, unsigned short packetid)
{
    ...
    ptr += MQTTSNPacket_encode(ptr, len); /* write length */
    writeMQTTSNChar(&ptr, packet_type);      /* write packet type */
        ...
        MQTTSNString* topicname)
{
    ...
    ptr += MQTTSNPacket_encode(ptr, len);  /* write length */
    writeMQTTSNChar(&ptr, MQTTSN_REGISTER);      /* write message type */
        ...
        unsigned char return_code)
{
    ...
    ptr += MQTTSNPacket_encode(ptr, len);  /* write length */
    writeMQTTSNChar(&ptr, MQTTSN_REGACK);      /* write message type */

    writeMQTTSNInt(&ptr, topicid);
    writeMQTTSNInt(&ptr, packetid);
    writeMQTTSNChar(&ptr, return_code);
