...
int MQTTSNPacket_encode(unsigned char* buf, int length)
{
    ...
    if (length > 255)
    {
        writeMQTTSNChar(&buf, 0x01);
        writeMQTTSNInt(&buf, length);
        rc += 3;
    }
    else
        buf[rc++] = length;
 ...
 */
int readMQTTSNInt(unsigned char** pptr)
{
 ...
 */
char readMQTTSNChar(unsigned char** pptr)
{
    char c = **pptr;
    (*pptr)++;
    return c;
 ...
 */
void writeMQTTSNChar(unsigned char** pptr, char c)
{
 ...
 */
void writeMQTTSNInt(unsigned char** pptr, int anInt)
{
 ...
 */
void writeMQTTSNCString(unsigned char** pptr, char* string)
{
...
int getMQTTSNLenStringLen(char* ptr)
{
    int len = 256*((unsigned char)(*ptr)) + (unsigned char)(*(ptr+1));
    return len;
}
...
void writeMQTTSNString(unsigned char** pptr, MQTTSNString MQTTSNString)
{
    if (MQTTSNString.lenstring.len > 0)
    {
        ...
        *pptr += MQTTSNString.lenstring.len;
        ...
        writeMQTTSNCString(pptr, MQTTSNString.cstring);
}
