...
class Converter {
     ...
     */
    function __construct ($path, $real_name, $out_name ,$dith, $cf, $outDir) {
     		
        $this->dith = $dith;
        $this->out_name = $out_name;
        $this->path = $path;
        $this->target_path = $outDir;
...
const lv_img_dsc_t " . $this->out_name . " = {
  ...
  .header.h = " . $this->h . ",\n";
    ...
    function download_c($name,$outpath="./", $cf = -1, $content = ""){
      global $offline;
    ...
    function download_bin($name, $outpath="./", $cf = -1, $content = 0){
      global $offline;
...
$conv = new Converter($img_file, $img_file_name, $output_name, $dith, $cf, $outDir);
...
if(!strcmp($format, "c_array")) {
    if(!strcmp($cf, "true_color") || !strcmp($cf, "true_color_alpha") || !strcmp($cf, "true_color_chroma")) {
        ...
        if(!strcmp($cf, "true_color")) $conv->download_c($conv->out_name, $conv->target_path, $conv::CF_TRUE_COLOR, $c_res);
        if(!strcmp($cf, "true_color_alpha")) $conv->download_c($conv->out_name, $conv->target_path, $conv::CF_TRUE_COLOR_ALPHA, $c_res);
        if(!strcmp($cf, "true_color_chroma")) $conv->download_c($conv->out_name, $conv->target_path, $conv::CF_TRUE_COLOR_CHROMA, $c_res);
    }
    else if(!strcmp($cf, "alpha_1")) {
         $conv->convert($conv::CF_ALPHA_1_BIT, 1);
         $conv->download_c($conv->out_name, $conv->target_path);
    }
...
/*Binary download*/
    ...
    if(!strcmp($cf, "true_color") || !strcmp($cf, "true_color_alpha") || !strcmp($cf, "true_color_chroma")) {
        ...
        if(!strcmp($cf, "true_color"))          $conv->download_bin($conv->out_name,$conv->target_path, $conv::CF_TRUE_COLOR);
        if(!strcmp($cf, "true_color_alpha"))    $conv->download_bin($conv->out_name,$conv->target_path, $conv::CF_TRUE_COLOR_ALPHA);
        if(!strcmp($cf, "true_color_chroma"))   $conv->download_bin($conv->out_name, $conv->target_path, $conv::CF_TRUE_COLOR_CHROMA);
    }
    else if(!strcmp($cf, "alpha_1")) {
         $conv->convert($conv::CF_ALPHA_1_BIT, 1);
         //$conv->download_bin($conv->out_name);
         $conv->download_bin($conv->out_name, $conv->target_path);
    }
    else if(!strcmp($cf, "alpha_2")) {
         $conv->convert($conv::CF_ALPHA_2_BIT, 1);
         //$conv->download_bin($conv->out_name);
